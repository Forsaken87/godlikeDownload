...
<div class='ui-widget ui-widget-header ui-corner-all'>
	<a href='index.php?show=<?=urlencode($page)?>&page=1' class="pager_button">Erste</a>
<?php
...
for ($cur_page = $limit_pagelist_first; $cur_page <= $limit_pagelist_last; $cur_page++) {
	...
	echo("	<a href='index.php?show=".urlencode($page)."&page=".$cur_page."' class='".$classes."'>".$cur_page."</a>\n");
}
if ($limit_pages > $limit_pagelist_last) {
	echo("<span>&nbsp;...&nbsp;</span>");
}
?>
	<a href='index.php?show=<?=urlencode($page)?>&page=<?=$limit_pages?>' class='pager_button'>Letzte</a>
</div>
